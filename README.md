...
`openbridge-probe` 不启动下游网关，也不修改注册表。它只对管理员显式选择、已经注册且已启用的一个 Upstream Target 执行基础
Models/Chat/Responses/Embeddings 探测，并输出脱敏 JSON：
...
可用选择器为 `--list-models`、`--chat`、`--responses`、`--embeddings` 和 `--all`。省略选择器时执行该 target 已注册的全部基础
