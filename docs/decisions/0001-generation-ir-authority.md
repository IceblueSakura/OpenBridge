...
- **实现：部分基础已存在，统一管线待实施。** 当前 Static/Event IR、codec 和固定路由可以复用；Native 的源 envelope 保留路径尚不等于 IR 驱动输出。
- **本轮范围：文档修订。** 本决策不表示 hook、工具执行器或管线重构已完成，也不独立授权代码变更。
...
现有代码已 decode Native 请求与响应，但请求仍可复制源对象后替换模型，静态响应仍可重发源 envelope。它有利于保存尚未建模的字段，却不能保证对 IR 的修改、删除和约束会反映到最终 wire。问题不是缺少 IR 类型，而是 IR 尚未成为完整的输出权威。
