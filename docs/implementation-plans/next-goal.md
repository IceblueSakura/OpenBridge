...
2. 继续关闭已有纯文本子集的缺口：Schema 方言、strict 准入与引用图资源预算、Chat response_format 映射、SDK 派生 view；审查其余字段/事件的必填性和拒绝边界。具体反例见[迁移缺口](../architecture-v2/migration.md#当前已知闭合缺口)。
...
[当前纯文本焦点](current-focus.md)仍未通过全部门槛。测试组织与依赖准备不修复已有语义缺口；当前 Responses 与单候选 Chat 的固定 SDK 局部验收范围见[开发指南](../development.md)和 [Chat profile](../architecture-v2/chat-text-profile.md)；后续按域补独立双向映射和静态/事件一致性，不扩大为重复模型矩阵。
