...
2. 继续关闭已有纯文本子集的缺口：SDK 派生 view；按实际目标完善 [Schema profile](../architecture-v2/schema-profile.md)，审查其余字段/事件的必填性和拒绝边界。具体反例见[迁移缺口](../architecture-v2/migration.md#当前已知闭合缺口)。
...
[当前焦点](current-focus.md)当前无进行中切片；下一步收敛为 SDK 派生视图回放：分别确定 Responses output_text.parsed 与 Chat parsed 消费对象的实际形态、原始正文权威、派生值一致性/失效规则与 static/event/history 各自的准入位置。其他纯文本缺口不随之消失。当前 Responses 与单候选 Chat 的固定 SDK 局部验收范围见[开发指南](../development.md)和 [Chat profile](../architecture-v2/chat-text-profile.md)；按域补独立双向映射和静态/事件一致性，不扩大为重复模型矩阵。
