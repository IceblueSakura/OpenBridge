...
`base_url` 是部署 endpoint base，而不只是 origin：在通过 origin allowlist 后，可包含受限的固定路径前缀。共享 transport 必须显式追加 adapter 生成的相对路径，不能使用会让 `/v1/...` 覆盖前缀的 URL join 语义。下游请求、adapter 和 credential 都不能改变该 endpoint base。
...
“受信服务所有者可配置 endpoint”与“业务请求不可指定 endpoint”必须明确区分。对本地 vLLM、兼容网关或自建 endpoint，前者是基础目标；后者是安全不变量。
