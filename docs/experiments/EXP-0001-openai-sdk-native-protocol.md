# EXP-0001：OpenAI SDK 原生 Chat/Responses tool-loop loopback
...
- Affected decision：作为 C0 的 SDK-first 前置证据；不影响 C0/C1 的真实 Codex/Hermes 接受门。
...
- Decision impact：SDK-first 证据已可重跑；C0 保持 `Active`，C1 保持 `Blocked by C0`，不可据此标为 Accepted。
- Required follow-up：保留 EOF/partial-stream/unknown event/cancel 的 Rust contract evidence；在 C0 固定 Codex/Hermes 版本并录制真实 corpus，C0 Accepted 后再为 C1 建立单阶段计划。
