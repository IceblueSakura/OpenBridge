...
- 每次 probe 为选中 target 单独加载 pool 快照，并确定性使用第一个 member，不参与生产 round-robin，也不修改 生产 cursor 或
  cooldown；
...
`--all`。`--target` 必须引用一个已启用的代码注册项。若 target 没有对应协议的 Upstream API，相关观察不会被 解释为 Provider
支持。
