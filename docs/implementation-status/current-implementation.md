...
| `mimo-v2.5` Chat/Responses Native 图片输入 | [native-image-input.md](features/native-image-input.md) |
| `mimo-v2.5` 音频理解与 MiMo 专用 ASR/TTS/VoiceDesign/VoiceClone Chat Native | [native-mimo-audio.md](features/native-mimo-audio.md) |
...
- [当前测试资产与保留标准](test-inventory.md)：记录业务测试入口、低价值测试排除规则与 canonical oracle 边界。
...
当前未完成或不在本轮实现范围的内容包括 Native file/video、OpenAI `/v1/audio/*`/Responses audio/Realtime、MiMo 音频模型经
OpenBridge 的真实端到端 Provider 验收、更广的 audio source/format/part 组合与其他模型图片能力、ChatGPT 的其他协议/工具/Agent loop、异构协议 Provider、OTLP
