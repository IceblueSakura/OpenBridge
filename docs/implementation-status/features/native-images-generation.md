...
- **错误矩阵**：400 `invalid_request_error` / `unsupported_model_capability`、404 `model_not_found`、
  ...
  502 `upstream_error`。
- **观测**：`request_kind="images"`、operation `images_generations`；body transport 与 invalid response 使用闭合低基数 error type，
  读取中取消终结为 cancelled，validated downstream drop 保持 Provider completed；原始 prompt、上游 body、URL 与 credential 不进入普通
  OTLP/metrics。显式开启的 bounded downstream content JSONL 仍按全局策略记录最终客户端 body，生产所有者须按敏感度关闭。
...
- Images 仍不复用可 recovery 的 Generation/Embeddings runner；deterministic synthetic tests 不证明真实 Provider 的内存峰值、SLA 或长期取消行为。
