...
- 当前公开契约：Chat/Responses 的文本、streaming 和 `PlainText` reasoning；模型 levels 为 `none`、`low`、`high`、`max`。
...
2026-08-10 根据上述真实关闭证据将 `none` 补入 canonical profile；生产 registry 的确定性测试确认 Chat/Responses Models 投影均为
`none/low/high/max`，且每个公开 level 都能生成执行计划。本次代码变更没有重复调用真实 Kimi Provider。
