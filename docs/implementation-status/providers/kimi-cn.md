...
- Chat/Responses 公开文本、streaming、`PlainText` reasoning 和 `low/high/max`；Kimi K3 始终启用 reasoning，不公开
  `none`。Public Model 的 `ClampPositiveFloor` 仍可接受其他正向输入档位并规范化到已确认档位，但明确拒绝 `none`。
...
[2026-08-09 文字矩阵](../evidence/real-provider/2026-08-09-text-generation-none-high-matrix.md)记录当时 `kimi-k3`
Chat/Responses × JSON/SSE × `none/high` 正常首选路径，成功单元均有完整终态；该历史观察不覆盖当前官方 Kimi K3
始终启用 reasoning 的合同，当前 Public Model 不再公开 `none`。
...
其他 Moonshot endpoint、原生 Responses、更多参数组合、账号权限、外部 SDK/Agent、负载和长期运行未证明。历史请求中
`none` 无可观察 reasoning 的结果只适用于记录中的部署、账号和时间点，不证明当前 Kimi K3 能关闭 reasoning。
