...
- 图片能力来自 2026-08-10 有界 probe：单张 PNG data URL 实测；JPEG 按 OpenAI-compatible convention 声明；remote URL ≤ 8192 字符，inline ≤ 20 MiB encoded / 15 MiB decoded，最多 4 张。这不证明多图或视觉质量。
- `logprobs`、`n`、`top_logprobs` 禁用；四个常规采样参数按 ignored-parameter 合同静默忽略。其他 Moonshot endpoint、原生 Responses、更多参数组合与账号权限未验证。
- 既有 `none` 观测不证明可关闭 reasoning。
