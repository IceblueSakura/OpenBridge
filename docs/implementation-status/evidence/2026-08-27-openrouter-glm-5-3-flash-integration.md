...
- 智谱 GLM-5.3-Flash 专属模型页：<https://docs.bigmodel.cn/cn/guide/models/vlm/glm-5.3-flash>。
...
- 官方专属页面声明 text/image/video/file input；OpenRouter endpoint 仅声明 text/image/video input、text output、1,048,576 context，以及 reasoning、tools、tool choice、response format 等参数。协议级字段仍需按实际 Chat/Responses wire 独立验证。
...
- canonical facts 记录官方 `file` 输入，但 OpenRouter target 的 executable file profile 保持关闭，因此下游不公开 file input。
...
本记录只证明当时账号、网络、OpenRouter 单一 Z.AI endpoint、最小合成 payload 和短输出。它不证明 direct Z.AI API、OpenRouter file input、remote image/JPEG、video、多图和大小上限、接近 1M 的长上下文、两个并行 tool call 的实际同时生成、复杂 tool loop、质量、价格、配额、其他账号/区域、负载、长期稳定性或未来 OpenRouter 行为。扩大能力前必须重新读取当前来源并执行对应真实请求。
