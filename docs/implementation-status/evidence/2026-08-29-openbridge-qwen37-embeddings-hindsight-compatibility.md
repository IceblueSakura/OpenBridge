...
- 本地验证对象：上述基线加 `src/providers/catalog/public_models.rs`、embedding 转换与 Bailian Responses 注册的工作区修订，并非纯基线 checkout；未读取或记录 credential 值
...
受测本地修订在 `bailian/qwen3-7-text-embedding` 的具体 Upstream API policy 中采用显式 wire translation：
