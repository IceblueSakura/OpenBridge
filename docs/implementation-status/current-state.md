...
| Native 图片输入 | `mimo-v2.5`、DeepSeek Vision 及 image-capable Bailian Qwen/Kimi Native interface 接受各自有界的 HTTPS URL 或规范 Base64 data URL；DeepSeek Vision 支持双 native 协议的 JPEG/PNG/GIF/WebP、显式 detail 与多图，Kimi Responses Bridge 不转换图片。 | `tests/forwarding_contract.rs` |
