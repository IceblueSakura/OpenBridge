...
| Native 图片输入 | `mimo-v2.5`、DeepSeek Vision、OpenRouter Gemini/Grok 及 image-capable Bailian Qwen/Kimi Native interface 接受各自有界的 HTTPS URL 或规范 Base64 data URL；OpenRouter 只为具有完整 executable image profile 证据的 Gemini 3.7 Flash 与 Grok 4.6 双 native 协议公开 JPEG/PNG，MiniMax M3 与 Gemma 4 保持 text-only 并在 egress 前拒绝图片；Bailian Qwen 公开 250 张 BMP/JPEG/PNG/TIFF/WebP/HEIC 上游 envelope，Bailian Kimi 保持单张 JPEG/PNG；DeepSeek Vision 支持 JPEG/PNG/GIF/WebP、显式 detail 与多图。 | `tests/forwarding_contract.rs` |
