...
本文只研究 Hermes 的 Chat/Responses mode、tool result、Provider 切换和 stream terminal。这里的 `codex_responses` 是 Hermes 对 OpenAI Responses 及若干兼容端点的内部模式名，不是一个对外 HTTP proxy 路由。
