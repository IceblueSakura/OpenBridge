...
- issue #20711 展示 Chat tool-call 首 chunk 带 id、后续 chunk 只带 index 时，缺少 `index -> call_id` 状态会丢 arguments
  delta。
...
这些 tests/issues 适合提供多 Provider 字段差异和负面回归样本；内部对象、静默跳过或全局 cache 补全是 LiteLLM
策略，不能被解释为协议标准。Provider adapter tests 也不单独证明双向转换的全部语义。
