...
model、账户与 Provider 约束，不能假定跨 model 或 Provider 可移植；自定义 identity 的创建见[自定义声音与 consent](audio-custom-voices.md)。
...
- file transcription 见 [Transcriptions](audio-transcriptions.md)；
- file translation 见 [Translations](audio-translations.md)；
- Chat audio 见 [Chat 音频输入/输出](audio-chat-input-output.md)；
