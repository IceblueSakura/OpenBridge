...
- Chat 图片输入见 [Chat 图片输入](images-chat-input.md)，两者不是字段级同构；
- Images generation/edit/variation 不由 `input_image` capability 推导；
- hosted image generation 是 Responses tool operation，见 [Responses hosted image generation](images-responses-hosted-generation.md)。
