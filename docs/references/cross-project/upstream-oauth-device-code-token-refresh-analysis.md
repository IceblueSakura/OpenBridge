...
CLIProxyAPI 是唯一持续运行后台调度器的样本；其他三个项目主要在 access token 被需要时检查 expiry。不同 safety window 是项目常量，不是 OAuth 标准推荐值。

## 5. refresh token rotation 的共同风险

RFC 6749 允许 authorization server 在 refresh 成功时返回新 refresh token；客户端必须丢弃旧值。RFC 9700 进一步要求 public client 使用 sender-constrained refresh token 或 rotation 检测重放。
...
Codex 的进程内 semaphore、CLIProxyAPI 的 mutex/singleflight、Hermes 的文件锁分别缓解局部并发；LiteLLM 的简单 JSON 覆盖写清楚展示了缺少协调时的竞态。四个样本都不能单独证明跨主机共享 credential 的一致性。
