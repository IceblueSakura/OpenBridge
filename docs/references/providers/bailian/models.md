...
2026-08-24 对 Qwen/DashScope 官方 Realtime Audio 文档的定向复核确认：`qwen-audio-3.0-realtime-plus` 与 `qwen-audio-3.0-realtime-flash` 使用原生实时 API，接受文本与音频并输出文本与音频，支持 Function Calling，不支持联网搜索或思考模式；会话最多保留 50 个音频 turns 或累计 300 秒音频。该时长限制不是 token context，不能换算为 token ceiling。
