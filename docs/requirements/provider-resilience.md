...
**Working core requirement；归属 C2 Provider 聚合核心。**

本文定义上游 Provider 因 RPM/TPM、并发限制、临时过载或服务故障返回 429/5xx 时的最小恢复行为。它补充[核心需求](proxy-requirements.md)中的 stream/fallback 约束，但不进入当前 C0 实施计划。
...
| 名称 | 含义 |
|---|---|
| Attempt | 对一个确定 deployment 的一次上游 HTTP 调用。 |
| Same-candidate retry | 在同一 deployment、credential binding 和协议路径上重新调用。 |
| Fallback | 按不可变 RoutePlan 转到下一个已批准 candidate。 |
| Cooldown | 在一个有界时间内阻止新的无状态请求选择某 deployment。 |
...
| `invalid_request` | 400/404/409/422 或 capability/schema 错误 | 否 | 否 | 否 |
...
4. 否则立即将 deployment 置为 cooldown，并评估下一个 candidate；
...
- reload 后仅在 deployment identity、credential binding 和 quota scope 未变化时保留 cooldown，否则丢弃旧状态。
...
{
  "error": {
    "message": "All eligible upstream deployments are temporarily rate limited or unavailable.",
...
| ID | 验收 |
|---|---|
| RES-01 | 429 + `Retry-After` 产生 deployment cooldown，新的无状态请求在到期前跳过它。 |
...
| RES-06 | `previous_response_id`、tool continuation 和 Provider resource 不跨 deployment。 |
...
相关源码调研见 [LiteLLM Proxy 请求链分析](../research/litellm/proxy-call-chain-analysis.md)和[性能瓶颈分析](../research/litellm/proxy-performance-bottlenecks.md)。

## 11. 关联文档

- [核心需求](proxy-requirements.md)
- [C2 Provider 聚合核心](../phases/02-provider-aggregation.md)
- [配置与路由模块](../modules/02-configuration-and-routing.md)
- [Provider Adapter 模块](../modules/03-provider-adapters.md)
