...
|---|---|---|---|---|
| Codex | 首要目标客户端、Responses 契约来源 | 已有源码调研；需固定新版本 corpus | Responses request/SSE、tools、continuation、custom Provider config、HTTP/SSE/WebSocket transport | 本地 auth cache、未公开 subscription backend |
| Hermes Agent | 首要目标客户端、多 transport Agent loop | 已有源码调研；需固定版本 E2E | Chat/Responses/Anthropic modes、tool loop、strict endpoint | 完整 Agent 产品和 Provider catalog UI |
...
| Bifrost | Provider core 与高性能 pipeline 的对照样本 | **P0 新增候选** | Provider interface、request flow、model catalog、native/compatibility split、cancellation | 企业治理、插件生态、性能宣传数字 |
| CLIProxyAPI | Codex/Chat/Responses bridge 负面案例库 | **P0 新增候选** | translator、tool identity、`previous_response_id`、stateful routing、SSE failures | 多账号/订阅 credential pooling、非官方 OAuth 路径 |

## 3. Codex

### 研究职责

- 定义 OpenBridge 的 P0 `/v1/responses` 下游契约；
...
- OpenBridge P0 使用独立 custom Provider id 并显式写入 `supports_websockets = false`，只承诺 HTTP/SSE。
- 旧版本曾出现内置 `openai` Provider 无法被同名配置覆盖的报告；OpenBridge 不依赖覆盖保留 Provider id，而是验证自己的 custom Provider profile。
- WebSocket 是否进入核心由固定 Codex 版本实验决定，不能从字段存在直接推导。

### 待补实验

- 固定一个 Codex release/commit；
...
- 定义 P0 Chat 与 P1 Responses 客户端契约；
...
```text
docs/research/bifrost/provider-core-and-translation-analysis.md
```

至少包含固定 commit、模块图、native/normalize path、Provider onboarding、三类负面 fixture 和 OpenBridge 结论。
...
```text
docs/research/cliproxyapi/codex-chat-responses-state-analysis.md
```
