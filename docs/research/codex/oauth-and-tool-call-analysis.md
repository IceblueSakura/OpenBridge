...
因此，真实 Codex OAuth 仍需完成[初版需求](../../requirements/proxy-requirements.md)定义的 `Codex OAuth preflight`，并取得官方文档、授权/条款确认和脱敏实测 fixture；在此之前只支持 mock OAuth adapter 或标准 API-key upstream。
