...
| History | system/developer text instructions, user/assistant text, assistant refusal, function calls and text tool results; replayed assistant `parsed` views validated against the raw body | No media, custom/hosted tools, third-party reasoning text or state resources; function `parsed_arguments` replay is not admitted |
...
The view is validated then dropped following the shared [derived replay rules](responses-text-profile.md#derived-replay-views): it never enters the IR, is never emitted, and a replaced body cannot resurrect it. Function `function.parsed_arguments` replay is not admitted here yet (the SDK's Chat `ParsedFunction` dumps it without the Responses-side exclusion), and the Responses `function_call` rule does not transfer automatically.
