...
2. 标准 configuration update 等续轮语义、更多 media/tool/event/state 分支没有实现；`summary:false`、cancelled event 等现有兼容分支需 profile 分类。SDK parsed text 与 function `parsed_arguments` 派生 view 的双协议回放准入见 [derived replay rules](responses-text-profile.md#derived-replay-views)。
