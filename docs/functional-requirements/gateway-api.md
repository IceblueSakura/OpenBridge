...
“请求可转发”不等于“某个 Agent 完整兼容”；客户端 profile、transport 和 tool loop 只有在本文明确声明时才构成承诺。

Chat/Responses Generation 的目标管线（双向 decode → 富语义 IR → encode，IR 为语义权威）是设计目标；
[ADR-0001](../decisions/0001-generation-ir-authority.md)说明决策依据，[Native Path 与流式语义](gateway-api/native-and-streaming.md)定义目标行为，当前实现差距见[实施状态](../implementation-status/current-boundaries.md)。
...
| [Native Path 与流式语义](gateway-api/native-and-streaming.md) | Native 基线、IR 语义权威目标、流式语义、遥测计时边界 |
