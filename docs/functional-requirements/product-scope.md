...
- 所选 Public Model 先按每协议唯一固定契约完成能力预检；通过后 Route 保持配置顺序，不按请求能力筛选或重排；
...
| `GET /openbridge/v1/models`、`GET /openbridge/v1/models/{model}` | 按[模型能力契约](model-information-and-capability-contract.md)返回同一目录的模型事实与每协议固定能力。 |
