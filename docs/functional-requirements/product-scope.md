...
本文定义 OpenBridge 当前产品目标、信任边界、客户端接口和明确非目标；实现完成度与验证边界由
...
`previous_response_id` 省略或为 `null`，`background` 省略或为 `false`。显式状态请求在任何 Provider egress 前拒绝，
OpenBridge 不提供 response 存储、后台 job、retrieve/cancel 或 continuation。

完整 envelope、Bridge 和 state-affinity 规则由[Generation envelope 与状态](gateway-api/generation-state.md)拥有。

## 3. 路由与静态装配

Provider、Canonical Model、Upstream Target、Upstream API、Route 与 Public Model 由受信 Rust 代码显式注册，
不从业务请求、Provider `/models`、文件扫描或插件动态生成。每个 generation Public Model 选择 `NativeFirst` 或
`SourceFirst`；能力预检通过后使用启动时冻结的 Route 顺序，不按请求能力、价格、健康或 Provider 名称筛选和重排。
具体排序与自动 Bridge 规则由[路由与 Provider 韧性](routing-resilience.md)拥有。

Registry、用户表、Route topology、API-key credential store 以及 OAuth manager 的 binding/locator/wiring 在启动后冻结；
OAuth token snapshot/generation 的受控刷新不能改变这些拓扑事实。
...
[当前开发焦点](../implementation-plans/current-focus.md)记录用户已授权的范围，再按变更工作流执行。
...
- 未由 [Provider OAuth 合同](configuration-credentials.md)明确规定的 OAuth 接入，以及 subscription 多账号池、账号级负载均衡、keyring 或远程 secret manager；
