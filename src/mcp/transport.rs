//! Streamable HTTP transport, JSON-RPC validation, and dispatch for the MCP server.
//!
//! This module implements only protocol revision `2026-07-28`. Tool definitions and execution live
//! behind the sibling tool registry so transport changes remain independent from tool growth. It
//! owns no Provider routing and rejects browser Origins, legacy sessions, and unregistered methods.
...
pub(crate) async fn endpoint(headers: HeaderMap, body: Bytes) -> Response {
