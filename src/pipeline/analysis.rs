...
pub fn analyze_embedding_request(
    body: &Bytes,
) -> Result<EmbeddingRequestRequirements, RequestPlanningError> {
    ...
    {
        return Err(RequestPlanningError::InvalidJson);
    }
    ...
    let requested_encoding = match object.get("encoding_format") {
        ...
        Some(_) => return Err(RequestPlanningError::InvalidJson),
    };
...
fn analyze_embedding_input(value: &Value) -> Result<EmbeddingInputAnalysis, RequestPlanningError> {
    ...
    let values = value.as_array().ok_or(RequestPlanningError::InvalidJson)?;
    if values.is_empty() {
        return Err(RequestPlanningError::InvalidJson);
    }
    let count = u32::try_from(values.len()).map_err(|_| RequestPlanningError::InvalidJson)?;
    ...
    if values.iter().all(Value::is_array) {
        let token_counts = values
            .iter()
            .map(|value| {
                let tokens = value.as_array().ok_or(RequestPlanningError::InvalidJson)?;
                if tokens.is_empty()
                    || tokens
                        .iter()
                ...
                u32::try_from(tokens.len()).map_err(|_| RequestPlanningError::InvalidJson)
            })
    ...
    Err(RequestPlanningError::InvalidJson)
}
